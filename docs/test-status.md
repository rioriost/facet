...
## Build 4: 会社名と組織・部署の個別共有（2026-09-16）

共通24テスト、12言語75キーの整合性確認、iPhone/Watchの署名付きRelease Archiveが成功。システムの連絡先選択と実画面のQR読取を含む5結合テストで、会社名・部署の個別選択、両方の出力、未選択情報の除外、保存・再起動、既存の手動更新・リセット動作を確認。日本語は「共有する項目」「共有設定」に統一。実機でのビルド4確認とTestFlight配信は未実施。詳細は[修正と検証記録](device-feedback-2.md)。
