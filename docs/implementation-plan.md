...
Apple Contactsを正本として、自分の連絡先を仕事用・プライベート用・双方の3つのQRプロファイルで公開する。初回起動と設定アイコンから、共有する連絡先と各プロファイルの公開フィールドを設定する。無料・MIT・広告なし・課金なし。QRの読取機能は実装しない。
...
- 共有する連絡先を変更した場合、すべての許可をリセットする。新たに追加されたContactsのフィールドは自動許可しない。
- iOSのマイカードを自動特定する公開APIに依存しない。システムの単一選択画面から、自分の連絡先1件を選ぶ。継続的アクセス権は要求しない。
- 連絡先の自動再読込はしない。選択した1件の対応項目を端末内に保存し、編集後は同じ連絡先を選び直す。設定画面・説明に手動更新を明記。既存build 1の設定のみ、許可済みなら選択済みIDを一度読み込み移行する。
...
- 個人情報をログに書かない。ファイルはApplication Supportに原子的保存、端末のファイル保護、バックアップ除外。iPhoneは選択した1件の対応フィールドとID、公開設定を保存する。写真・メモ等と他の連絡先は保存しない。リセット時はこのコピーも消去する。
...
ユーザーの選択によりbuild 2から最短の1件選択と手動更新を採用。初期設計の権限要求と自動更新を置換した。詳細は[実機フィードバック](device-feedback-1.md)を参照。
