...
## 会社名・組織／部署と共有表現の修正: 1.0.0 (4)

2026-09-16:

- ソース `53045d1`。会社名と組織・部署を独立して選択できるよう修正し、日本語の「公開」を連絡先の「共有」という表現へ変更。
- 共通テスト24件、Contacts結合テスト5件、12言語75キーの検証と署名付きRelease Archiveが成功。
- 12:27 JSTにアップロード成功。Apple側の処理完了後、既存内部グループ `facet test` でビルド4の「テスト中」（残り90日）を確認。
- テスト内容に、更新後は同じ連絡先を選び直して「組織・部署」を各プロファイルで選択する手順を保存。既存のテスターとグループを維持。
- 12:47 JSTに公開審査へ再提出し、1.0.0 (4)の「審査待ち」を確認。承認後の自動リリース設定を維持。
- [提出記録・確認範囲](app-review-build-4-2026-09-16.md)。
