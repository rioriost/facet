...
Facetは連絡先をiPhone内で処理し、選択した項目のQRのみをApple Watchへ同期します。開発者による連絡先の収集、広告、解析、課金はありません。仕事用・私用・双方の項目はそれぞれ明示的に選択します。

Watchはオフライン表示のためQRを保存します。iPhone側で削除しても通信できるまでは旧QRが残るため、必要に応じてWatch上で消去してください。読み取った相手が保存した情報は取り消せません。設定からのリセット、Watch上での消去、iOS設定での権限取消が利用できます。お問い合わせの公開Issueには個人情報やQRを添付しないでください。
